  ...
  Widget build(BuildContext context) {
    ...
    return FScaffold(
      header: FHeader.nested(
        title: Text(
          widget.poolType == BankPoolType.wrong
              ? 'Wrong answer bank'
              : 'Skipped bank',
        ),
      ...
      // bar/notch; this only needs to keep the bottom button clear of the
      // gesture nav bar.
      child: SafeArea(
        top: false,
        ...
        child: questionsAsync.when(
          ...
          data: (questions) => answersAsync.when(
            loading: () => const Center(child: FCircularProgress()),
            error: (err, stack) =>
                ErrorState(error: err, label: 'Failed to load attempt history'),
            data: (answers) {
              ...
              final topics =
                  pool.map((q) => q.topic).whereType<String>().toSet().toList()
                    ..sort();
              final difficulties =
                  pool
                      .map((q) => q.difficulty)
              ...
              final filtered = pool.where((q) {
                if (_topicFilter != null && q.topic != _topicFilter) {
                  return false;
                }
              ...
              return Column(
                ...
                children: [
                  ...
                  // Expanded + scroll view rather than Expanded(FTileGroup)
                  // directly, so a short filtered list sizes to its own
                  // content instead of stretching the tile group's box to
                  ...
                  if (filtered.isNotEmpty) ...[
                    ...
                    FButton(
                      ...
                      child: Text(
                        'Start exam with these ${filtered.length} questions',
                      ),
                    ),
                  ],
                ],
